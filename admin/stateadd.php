...
if (isset($_POST["insert"])) {
    ...
    if (!empty($state)) {
        $sql = "SELECT * FROM state WHERE sname = '$state'";
        $result = $con->query($sql);

        if ($result->num_rows > 0) {
            // Data already exists, handle accordingly (e.g., display an error message)
            $error = "<p class='alert alert-warning'>* State is already exist</p>";
        } else {
			...
			if ($result) {
				$msg =
					"<p class='alert alert-success'>State Inserted Successfully</p>";
			} else {
		...
		<!-- Main Wrapper -->
           ...
           while ($row = mysqli_fetch_row($query)) { ?>
                                                <tr>
													...
													<td><a href="stateedit.php?id=<?php echo $row[
                 "0"
             ]; ?>"><button class="btn btn-info">Edit</button></a>
                                                    <a href="statedelete.php?id=<?php echo $row[
                                                        "0"
